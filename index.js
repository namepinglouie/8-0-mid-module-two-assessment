...
function filterByGenre(movies, genre) {
  ...
  return movies.filter(movie => {
    if(movie.genre.toLowerCase().includes(genre.toLowerCase())) return movie;
  })
}
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  return movies.filter(movie => {
    ...
    let movieYear = splitYear[2];
    if(movieYear <= year) return movie;
  })
}
...
function getRottenTomatoesScoreByMovie(movies) {
  if(!movies.length) throw "There are no movies";
  return movies.map(movie => {
    let ratingObj = movie.ratings.find(rating => rating.source === "Rotten Tomatoes");
    let movieObj = {};
    movieObj[movie.title] = ratingObj.value;
    return movieObj;
  })
}
